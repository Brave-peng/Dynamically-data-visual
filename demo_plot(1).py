...
headers=['date','tid','open','high','low','close','volume','mid','range','fac0','avg','s3','s7','s14','s21','s35','s49',
's75','s100','s150','s200','s300','s400','s500','s600','s800','s1000','s1500','s2000','s3000','ma_close','ub','lb']

lm_hd=['s3000','s2000','s1500','s1000','s800','s600','s500','s400','s300','s200','s150','s100','s75','s49','s14','ub','lb']
...
def pic_out(if_mirror):
    ...
    #for comm in stock_pool_list_result:
        ...
        #plot各个级别线条以及修改线条参数
        close_px=rawd3.sort_index(ascending=True).close.plot(linewidth=0.7,label='close',color='#767676',secondary_y=True)
        close_pxma=rawd3.sort_index(ascending=True).ma_close.plot(linewidth=2,label='ma_close',color='#767676',secondary_y=True)
        ...
        s100=rawd3.sort_index(ascending=True).s100.plot(linewidth=2.3,label='s100',color='#FFFF00',secondary_y=False)
        s150=rawd3.sort_index(ascending=True).s150.plot(linewidth=4.5,label='s150',color='#FF0000',secondary_y=False) #original_lwt==4
        s200=rawd3.sort_index(ascending=True).s200.plot(linewidth=4.5,label='s200',color='#000000',secondary_y=False) #original_lwt==4
        s300=rawd3.sort_index(ascending=True).s300.plot(linewidth=2.1,label='s300',color='#800080',secondary_y=False)
        s400=rawd3.sort_index(ascending=True).s400.plot(linewidth=2.1,label='s400',color='#800080',secondary_y=False)
        s500=rawd3.sort_index(ascending=True).s500.plot(linewidth=2.1,label='s500',color='#800080',secondary_y=False)
        s600=rawd3.sort_index(ascending=True).s600.plot(linewidth=2.1,label='s600',color='#800000',secondary_y=False)
        s800=rawd3.sort_index(ascending=True).s800.plot(linewidth=2.1,label='s800',color='#993300',secondary_y=False)
        s1000=rawd3.sort_index(ascending=True).s1000.plot(linewidth=4.5,label='s1000',color='#9090FF',secondary_y=False) #original_lwt==4.2
        s1500=rawd3.sort_index(ascending=True).s1500.plot(linewidth=2.1,label='s1500',color='#FF99CC',secondary_y=False) #original_lwt==1.8
        s2000=rawd3.sort_index(ascending=True).s2000.plot(linewidth=2.1,label='s2000',color='#FFCC99',secondary_y=False) #original_lwt==1.8
        s3000=rawd3.sort_index(ascending=True).s3000.plot(linewidth=4.5,label='s3000',color='#FF6600',secondary_y=False) #original_lwt==4.2
        ...
        if if_mirror == True:
            plt.savefig('C:\\Users\\anton\\Desktop\\demo_000001_30.png',format='png',dpi=200,facecolor='lightsteelblue')
        else:
            plt.savefig('C:\\Users\\anton\\Desktop\\demo_000001_a.png',format='png',dpi=200,facecolor='lightsteelblue')
